...
Important product/workflow areas currently discussed in the project include:

- Main UI / workflow control
- Amazon product crawler
- Customize crawler / customization-data retrieval
- SEO + Content
- Auto SEO
- Pinterest
...
## Important: `module-seo-content` current state

`src/modules/module-seo-content/` exists, but in the current snapshot it is still essentially copied scaffold code based on Module B.

For example, its current public types are still named similar to:
...
and the current placeholder service only counts input items.

Therefore:

> `module-seo-content` is **not yet implemented according to the real SEO + Content business contract**.

Do not treat its current placeholder names or behavior as the intended domain design.

Before implementing real SEO + Content behavior, replace/refactor that scaffold into domain-specific types and logic.
...
> Convert product images and raw content into structured product understanding.

B1 should inspect product visual information, including where useful:

- OCR / text visible on the design;
- important visual objects/elements;
- colors;
- theme;
- style/aesthetic;
- personalization signals;
- other visible product characteristics.
...
`module-seo-content` was added after the original handbook scope list. If the team has not yet approved a branch scope name for it, do not silently invent a convention when branch naming is important; use a leader/user-provided branch name or coordinate the scope update.
...
# 39. Current SEO + Content implementation priorities

As of the current project discussion, the recommended order is:

```text
Phase 1
- finalize SeoContentInput / SeoContentOutput
- replace copied Module B scaffold names
- make mock/real contract compile

Phase 2
- build B1/B2 internal data model
- define seed keyword generation

Phase 3
- implement B3 Google Autocomplete API path
- normalize and preserve suggestion provenance

Later
- Etsy Suggest Scraper
- PAA collection
- B4 conflict/vector mapping
- B5 content planning/generation
- B6 WebP + alt/filename processing
```

This sequence is directional, not a hard schedule. If the leader defines another implementation order, follow the explicit task.
...
- current `module-seo-content` placeholder contract is correct;
...
FFP Tool is a single React/TypeScript/Vite application organized into independently owned business modules connected by an orchestrator. Each module exposes a stable input/output contract through its own `index.ts`, provides matching real and mock runners, and must not depend directly on another business module's internals. The target product automates Shopify-oriented product workflows including crawling, customization retrieval, SEO/content optimization, Auto SEO, Pinterest, and Shopify API synchronization. The SEO + Content module currently has a placeholder scaffold but its intended contract receives product images, niche, current title/description, and handle, then returns optimized product title/description, SEO title/description, WebP images with alt text, and an optimized handle. Internally it is expected to perform product/image understanding, audience/buying-context analysis, search-suggestion collection (Google Autocomplete first; Etsy Suggest and PAA later/alongside as planned), SEO conflict control, search-intent-driven content allocation/generation, and image optimization. The project enforces strict TypeScript, module ownership, public API boundaries, mock/real parity, deterministic tests, orchestrator-managed dependencies, secure environment handling, and focused branch-based development.
